from flask import Flask, Blueprint, session, redirect, render_template, url_for, request, jsonify, current_app
...
def communicationModule():
    ...
    # cursor.execute('SELECT * FROM `ceo_login_database`.ceo_login_database')
    cursor.execute('SELECT username FROM `ceo_login_database`')
    userName = cursor.fetchall()
    username_array = [user['username'] for user in userName]
